...
void addAllMoves(int grid[GRID_SIZE][GRID_SIZE], std::vector<short> &moves, int pieceCount, int pieceIndex, Piece piece) {
	...
	for (uint i = 0; i < pieceOrientations.size(); i++) {
		...
		// std::cout << "piece: \n";
		// pieceOrientation->printPiece();
		// pieceOrientation->printBoundingBox();
...
bool checkValidPlacement(int grid[GRID_SIZE][GRID_SIZE], Piece piece, int startI, int startJ) {
	for (int i = 0; i < piece.effectiveHeight(); i++) {
		for (int j = 0; j < piece.effectiveWidth(); j++) {
			if (piece.getCellRelative(i, j) != 0 && piece.getCellRelative(i, j) != grid[startI + i][startJ + j]) return false;
		}
	}
...
std::vector<Piece> getUniqueOrientations(Piece piece) {
	...
	// Use the wood grain to reduce possible orientations. Only check 180 degree rotations.
	piece.rotate();
	piece.rotate();
